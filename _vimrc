...
if(has("win32"))
  ...
  augroup Vimrc_BackupVimFile
    ...
    augroup end
      ...
      function! KeepPumClosed()
        " We must make YCM believe that the cursor moved, otherwise it would
        " reopen the completion menu if there is more than one candidate
        let pos = getpos('.')
        call setpos('.', pos[0:1] + [1,0])
        do <nomodeline> youcompleteme CursorMovedI
        call setpos('.', pos)
      endfunction

      augroup YCM_ClosePumHack
        au CompleteDone * call KeepPumClosed()
      augroup END
      ...
      let g:ctrlp_extensions = ['dir', 'funky', 'undo']

      let g:ctrlp_map = '<C-p>f'
      nnoremap <C-p>b :CtrlPBuffer<CR>
      nnoremap <C-p>B :execute 'CtrlPBuffer ' . expand('<cword>')<CR>
      nnoremap <C-p>m :CtrlPMRU<CR>
      nnoremap <C-p>M :execute 'CtrlPMRU ' . expand('<cword>')<CR>
      nnoremap <C-p>d :CtrlPDir<CR>
      nnoremap <C-p>D :execute 'CtrlPDir ' . expand('<cword>')<CR>
      nnoremap <C-p>f :CtrlPFunky<CR>
      nnoremap <C-p>F :execute 'CtrlPFunky ' . expand('<cword>')<CR>
      nnoremap <C-p>u :CtrlPUndo<CR>
      nnoremap <C-p>U :execute 'CtrlPUndo ' . expand('<cword>')<CR>
